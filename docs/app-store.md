...
`supabase/migrations/2026-09-11_subscriptions.sql` を SQL Editor で実行する。
`subscriptions` テーブル、`household_entitled()`、recipes / meal_plans への
restrictive ポリシーが作られる。
...
コードには入っていない、公開に必須の項目。

| 項目 | なぜ必要か |
|---|---|
| **ソロモード** | Guideline 4.2。2人前提のままだと「機能が限定的」で弾かれる。1人でも成立させ、割り勘タブは1人なら隠す |
| **アカウント削除機能** | Guideline 5.1.1(v)。アカウント作成があるアプリは必須。設定から削除できること |
| **審査用デモアカウント** | Google サインインがあるので必須。課金済み状態にしておく（`subscriptions` に手で active の行を入れる） |
