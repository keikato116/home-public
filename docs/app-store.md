...
- パートナーが残る場合は、2人で貯めた共有データ（レシピ / 割り勘 / 共有の買い物リスト）を
  **相手に引き継ぐかどうかをカテゴリ単位で選べる**。既定は全部引き継ぐ
- 自分にしか見えないデータは、選択によらず必ず消える

共有データに作成者の記録が無い（`shopping_items` の共有アイテムにも split 系にも
作成者列が無い）ので、「自分の分だけ消す」はできない。カテゴリ単位で全部残すか
全部消すかの二択になり、消す側を選ぶと相手の画面からも消える。
確認画面にその旨を明示している。
