...
### 1. Supabase（公開版は専用プロジェクトを使う）

**個人用（彼女と2人で使っている web app）とは別のプロジェクトを作る。**

同居させると、公開版の都合で入れたルールが個人用のアプリに効いてしまう。
実際、課金ゲートを入れた時点で個人用のレシピと献立が保存できなくなり、
自分たちに手で課金済みの行を入れて回避する必要があった。
公開版はまだ利用者0人なので、分けるならデータの引っ越しが要らない今が一番安い。

手順:

1. Supabase で新規プロジェクトを作る
2. そのプロジェクトの SQL Editor で `supabase/schema.sql` を実行する
3. 続けて下のマイグレーションを順に実行する
4. 公開版の Vercel プロジェクトに、新しいプロジェクトの URL とキーを設定する
5. 個人用のプロジェクトから公開版のルールを外す
   → `supabase/rollback/2026-09-11_revert_public_edition.sql`
   （公開版のプロジェクトでは絶対に流さないこと）

SQL Editor には**ファイルの中身**を貼る。ファイルのパスを貼っても実行できない。
...
**配信は日本のみにする。** App Store Connect の「価格および配信」で日本だけを選ぶ。
あとから増やせるが、EU を入れるときは下記を承知のうえで判断すること。

### 何が公開されるか

| | 公開 | 対処 |
|---|---|---|
| **本名**（販売元 / Seller） | される | 避けられない。個人で登録する以上、戸籍名がアプリページに出る。屋号・ニックネーム・別名は不可。法人名にするには法人を設立して Organization として加入するしかない |
| **住所・電話番号** | **EU に配信した場合のみ** | **日本限定配信で回避**（方針） |
| サポート用メール | される | 個人の Gmail をそのまま出すと素性が推測される。独自ドメインの転送アドレスを用意する |
| ドメインの登録者情報 | 代行が無ければ公開 | **Whois 情報公開代行を取得時に有効化**。お名前.com は後付けが有料なので取得時に必ず入れる |

### EU 配信の条件（DSA）

EU のデジタルサービス法により、**有料アプリまたはアプリ内課金があるアプリ**の開発者は
「取引者（trader）」とみなされ、**氏名・住所・電話番号・メールアドレスがアプリページに
公開される**。2025年2月の期限で、未提出のアプリは EU の App Store から一斉に削除された。

初回リリースは課金なしなので直ちには対象外だが、**あとでサブスクを入れた時点で対象になる**。
日本限定である限り適用されないので、課金を入れるときも配信国は日本のままにしておくこと。
