...
**Google のトークンは保存前に暗号化している（2026-09-15）。**
鍵は `TOKEN_ENCRYPTION_KEY`（Vercel の環境変数）。Supabase の外に置くことに
意味があるので、**DB 側に鍵を持たせないこと**。守れるのは「DB のダンプだけが
流出したとき」で、Vercel と Supabase の両方を持つ運営者からは守れない。
そこまで隠すには端末間の暗号化が要り、鍵を失うと復旧できなくなる。

鍵はサーバーにしか置けないので、**トークンの読み書きは `/api/tokens` を通す。**
ブラウザから `user_tokens` を直接読み書きしないこと（暗号文が
localStorage に入り、リフレッシュが静かに壊れる）。暗号化を入れる前に
保存された行は平文で、`decryptToken` がそのまま返す。移行は不要で、
次に書かれた時点で暗号文になる。
