...
> **進捗 (2026-06-12)**: フェーズ0–7 すべて完了。
> フェーズ6の DB migration SQL は `supabase/migrations/2026-06-12_phase6_legacy_cleanup.sql` —
> **Supabase ダッシュボード > SQL Editor で手動実行が必要**（コードは実行前後どちらでも動く両対応）。
> SQL 実行後に確認が取れたら、`isHighlightPlan` のレガシー分岐と `__ratio__` 書き込みを削除できる。
